...
serve_df.loc[:, 'PointWon'] = (serve_df['PointWinner'] == serve_df['ServeIndicator']).astype(int)
...
![](images/court.jpeg){style="transform: rotate(90deg); width=100%; length=200%"}
</div>
...
### Does the server maintain their advantage throughout the entire game?
...
sns.lineplot(x=avg_server_win_rate.index, y=avg_server_win_rate.values, marker='o', linestyle='-', label='Average')

plt.axhline(0.5, color='red', linestyle='dashed', label='50% (Neutral)')
plt.title('Average Server Win Rate Over Match Time', fontsize=15)
plt.xlabel('Elapsed Time (minutes)', fontsize=15)
plt.ylabel('Server Win Rate', fontsize=15)
plt.xlim(0, 200)
plt.legend()
plt.grid()
plt.show()
...
```{python}
#| echo: true
winner_distances = df2[df2['WonSet'] == 1]['P1DistanceRun'].tolist() + df2[df2['WonSet'] == 2]['P2DistanceRun'].tolist()
loser_distances = df2[df2['WonSet'] == 1]['P2DistanceRun'].tolist() + df2[df2['WonSet'] == 2]['P1DistanceRun'].tolist()
...
plt.figure(figsize=(14, 8))
sns.boxplot(data=data, palette=["lightblue", "red"])
...
```{python}
#| echo: true
winners_distance = df2[df2['WonSet'] == 1]['P1DistanceRun'].tolist() + df2[df2['WonSet'] == 2]['P2DistanceRun'].tolist()
losers_distance = df2[df2['WonSet'] == 1]['P2DistanceRun'].tolist() + df2[df2['WonSet'] == 2]['P1DistanceRun'].tolist()

t_stat, p_value = stats.ttest_ind(winners_distance, losers_distance, equal_var=False)
...
for i in range(1, x+1):
    adv_df = df2[((df2['P1GamesWon'] == i) & (df2['P2GamesWon'] == 0)) | ((df2['P2GamesWon'] == i) & (df2['P1GamesWon'] == 0))]
    wins = ((adv_df['P1GamesWon'] == i) & (adv_df['P2GamesWon'] == 0) & (adv_df['WonSet'] == 1)).sum() + \
           ((adv_df['P2GamesWon'] == i) & (adv_df['P1GamesWon'] == 0) & (adv_df['WonSet'] == 2)).sum()
